...
async function boundedText(stream: ReadableStream<Uint8Array>) {
  ...
  try {
    while (true) {
      ...
      if (size > 65536) {
        await reader.cancel();
        throw new Error("version_output_limit");
      }
...
async function upsertAsset(
  value: Omit<Asset, "id" | "createdAt" | "updatedAt">,
) {
...
async function scanSource(
  ...
  const configPaths =
    root.id === "claude"
      ? [join(homedir(), ".claude.json"), join(root.home, "settings.json")]
      : root.id === "codex"
      ...
      if (executable) {
        ...
        try {
          ...
          try {
            ...
            version = output.trim().slice(0, 120) || null;
          } finally {
        ...
        await upsertAsset({
          ...
          description: null,
          status: "available",
          metadata: { configHome: root.home },
        });
        result.agents++;
      }
      await scanSource(root, result);
    }
    const skills = await scanAssets(undefined, context);
    result.skills = skills.skills;
    result.limited ||= skills.limited;
    const sessions = await scanSessions(undefined, context);
    result.sessions = sessions.files;
    result.parsedEvents = sessions.events;
    result.limited ||= sessions.limited;
    await audit("registry.scanned", "local", {
      ...result,
      errors: result.errors.length + sessions.errors,
    });
    return result;
  } finally {
    scanning = false;
