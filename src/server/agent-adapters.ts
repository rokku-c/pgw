...
export interface TurnResult {
  status: "completed" | "failed" | "interrupted" | "blocked";
  text: string;
  error?: string;
}
...
export async function createAdapter(
  ...
  env.PGW_MCP_KEY = key;
  const hasMcp =
    !!run.controls.memoryAccess || !!run.controls.mcpGrants?.length;
  const output = (text: string) =>
    hooks.output(text.replaceAll(key, "[redacted]"));
  let native: AgentProcess | undefined;
  let session = run.nativeSessionId;
  let turnId = run.nativeTurnId;
  let current:
    { resolve: (result: TurnResult) => void; text: string } | undefined;
  const finish = (result: Omit<TurnResult, "text">) => {
    const pending = current;
    current = undefined;
    pending?.resolve({ ...result, text: pending.text });
  };
  const failure = (error: Error) =>
    finish({ status: "failed", error: error.message });
  const append = (text: string) => {
    output(text);
    if (current) current.text = (current.text + text).slice(-250000);
  };
  const observeExit = (child: AgentProcess) => {
    void child.exited.then(async (code) => {
      await child.drained();
      if (native === child && current)
        finish({ status: "failed", error: `agent_exited:${code}` });
    });
  };
  if (run.agent === "codex") {
    ...
    await Bun.write(configPath, config + mcpConfig);
    await chmod(configPath, 0o600);
    env.CODEX_HOME = dir;
    native = new AgentProcess(
        ...
        onEvent: async (event) => {
          ...
          if (event.id !== undefined && event.method) {
            await hooks.approval(
              { id: event.id, method: event.method, params: p },
              (result) => native!.send({ id: event.id, result }),
            );
            return;
          }
          if (event.method === "turn/started") {
            turnId = p.turn.id;
            await hooks.identity(session!, turnId);
          } else if (event.method === "item/agentMessage/delta")
            append(p.delta || "");
          else if (event.method === "item/commandExecution/outputDelta")
            output(p.delta || "");
          else if (event.method === "item/completed")
            await hooks.event("native.tool", {
              id: p.item?.id,
              type: p.item?.type,
            ...
            await hooks.event("native.request_resolved", {
    ...
    try {
      ...
      const response = await native.request(
        session ? "thread/resume" : "thread/start",
        {
          ...
          sandbox: run.controls.permission,
        },
      );
    ...
    if (run.controls.permission !== "read-only")
      ...
      ],
      {
        ...
        onEvent: async (event) => {
          if (
            ...
            event.type &&
            /(tool|function|command|mcp)/i.test(event.type)
          )
            await hooks.event("native.tool", {
              type: event.type,
              phase: /(result|end|complete|done|output)/i.test(event.type)
                ? "result"
                : "pending",
            });
            ...
            await hooks.approval(
              { id: event.id, method: "pi/input", params: event },
              (response) =>
                native!.send({
            ...
            finish({
              status: "failed",
              error: event.message.errorMessage || "agent_error",
            });
        },
      },
    );
    observeExit(native);
    try {
      const state = await native.request("get_state", {}, true);
  ...
  return {
    async turn(message) {
      ...
      if (run.agent === "codex") {
        try {
          const response = await native!.request("turn/start", {
            threadId: session,
        ...
        try {
          await native!.request("prompt", { message }, true);
        } catch (error) {
          failure(error as Error);
        }
      } else {
        ...
        env.ANTHROPIC_DEFAULT_SONNET_MODEL = route.alias;
        env.ANTHROPIC_DEFAULT_OPUS_MODEL = route.alias;
        env.ANTHROPIC_DEFAULT_HAIKU_MODEL = route.alias;
        ...
        const args = [
          ...
          ...(run.controls.permission === "read-only"
              ...
              for (const part of event.message?.content || []) {
                if (part.type === "text") append(part.text + "\n");
                  ...
                  await hooks.event("native.tool", {
                    type: part.type,
                    name: part.name || null,
                    phase: part.type === "tool_result" ? "result" : "pending",
                  });
              }
    ...
    async steer(message) {
      ...
      if (run.agent === "codex")
        await native!.request("turn/steer", {
          threadId: session,
          expectedTurnId: turnId,
          input: [{ type: "text", text: message, text_elements: [] }],
        });
      else if (run.agent === "pi")
        await native!.request("steer", { message }, true);
      else throw new ApiError(409, "steering_not_supported");
    },
    async interrupt() {
      if (!current) return;
      try {
        if (run.agent === "codex" && turnId)
          await native!.request("turn/interrupt", {
            threadId: session,
            turnId,
          });
        else if (run.agent === "pi") {
          await native!.request("clear_queue", {}, true);
          await native!.request("abort", {}, true);
