...
export const home = resolve(
  process.env.PGW_HOME || join(homedir(), ".personal-gateway"),
);
mkdirSync(home, { recursive: true, mode: 0o700 });
chmodSync(home, 0o700);
export const port = Number(process.env.PGW_PORT || 7210);
if (!Number.isInteger(port) || port < 1 || port > 65535)
  throw new Error("Invalid PGW_PORT");
export const address = `http://127.0.0.1:${port}`;
export const configDatabasePath = join(home, "config.sqlite");
...
function persistentSecret(name: string, length: number): Buffer {
  ...
  if (!existsSync(path)) {
    try {
      writeFileSync(path, randomBytes(length), { mode: 0o600, flag: "wx" });
    } catch (error) {
...
export const adminToken = persistentSecret("admin.key", 32).toString(
  "base64url",
);

export const devAccessPath = resolve(
  import.meta.dir,
  "../../.gateway",
  port === 7210 ? "dev-access.json" : `dev-access-${port}.json`,
);
export function writeDevAccessFile() {
  ...
  if (!info.isDirectory() || info.isSymbolicLink())
    throw new Error("Invalid development access directory");
  ...
  try {
    writeFileSync(
      temporary,
      JSON.stringify(
        { url: `${address}/#token=${adminToken}`, token: adminToken },
        null,
        2,
      ) + "\n",
      { mode: 0o600, flag: "wx" },
