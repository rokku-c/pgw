...
export class BunSqlite {
  ...
  constructor(filename: string, options: { readonly?: boolean; fileMustExist?: boolean; timeout?: number } = {}) {
    this.database = new Database(filename, { readonly: options.readonly, create: !options.fileMustExist, strict: true });
    this.database.run(`PRAGMA busy_timeout = ${Math.max(0, Math.trunc(options.timeout ?? 5000))}`);
  }
  prepare(sql: string) {
  ...
  pragma(sql: string) { return this.database.prepare(`PRAGMA ${sql}`).all(); }
