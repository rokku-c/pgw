...
export function endpoint(provider: Provider, path: string) {
  ...
  if (provider.protocol === "gemini")
    return `${base.endsWith("/v1beta") ? base : `${base}/v1beta`}${suffix}`;
...
export async function probeProvider(provider: Provider) {
  ...
  const update = {
    health: result._tag === "Right" ? ("up" as const) : ("down" as const),
    latencyMs,
    lastChecked: Date.now(),
    updatedAt: Date.now(),
    lastError:
      result._tag === "Left" ? result.left.message.slice(0, 160) : null,
  };
