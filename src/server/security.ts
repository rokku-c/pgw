...
export function encrypt(secret: string) {
  ...
  return Buffer.concat([iv, cipher.update(secret, "utf8"), cipher.final(), cipher.getAuthTag()]).toString("base64url");
}
...
export function bearer(request: Request) {
  return request.headers.get("authorization")?.match(/^Bearer\s+(\S+)$/i)?.[1] || request.headers.get("x-api-key") || "";
}
export function checkLocalRequest(request: Request) {
  ...
  if (origin && !allowed.some(host => origin === `http://${host}`)) throw new ApiError(403, "origin_denied");
}
const cookieSignature = (value: string) => createHmac("sha256", encryptionKey).update(value).digest("base64url");
...
export function isAdmin(request: Request) {
  ...
  const raw = request.headers.get("cookie")?.split(";").map(s => s.trim()).find(s => s.startsWith("pgw_session="))?.slice(12);
  if (!raw) return false;
  const [expires, nonce, signature] = raw.split(".");
  return Number(expires) > Date.now() && !!nonce && !!signature && secureEqual(signature, cookieSignature(`${expires}.${nonce}`));
}
...
export function newClientKey() { return `pgw_${randomBytes(32).toString("base64url")}`; }
