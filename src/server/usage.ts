...
function count(value: unknown): number | null {
  ...
  if (typeof value !== "number" || !Number.isSafeInteger(value) || value < 0)
    throw new ApiError(502, "invalid_upstream_usage");
  return value;
}
export function mergeUsage(
  ...
  if (input !== null)
    result.inputTokens =
      protocol === "messages"
        ? input + result.cacheReadTokens + result.cacheWriteTokens
        : input;
  else if (protocol === "messages" && previous.inputTokens !== null)
    result.inputTokens =
  ...
  if (output !== null)
    result.outputTokens =
      protocol === "gemini" ? output + result.reasoningTokens : output;
  else if (protocol === "gemini" && previous.outputTokens !== null)
    result.outputTokens =
      previous.outputTokens - previous.reasoningTokens + result.reasoningTokens;
  if (
    [result.inputTokens, result.outputTokens].some(
      (value) => value !== null && (!Number.isSafeInteger(value) || value < 0),
    )
  )
    throw new ApiError(502, "invalid_upstream_usage");
  if (
    result.cacheWriteLongTokens > result.cacheWriteTokens ||
    (result.inputTokens !== null &&
      result.cacheReadTokens + result.cacheWriteTokens > result.inputTokens) ||
...
export function usageCost(usage: Usage, rates: PriceSnapshot): number | null {
  ...
  const parts: [number, number | null | undefined][] = [
    ...
    [usage.cacheWriteTokens - usage.cacheWriteLongTokens, rates.cacheWrite],
    [usage.cacheWriteLongTokens, rates.cacheWriteLong],
  ];
  let result = 0n;
