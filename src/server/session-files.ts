...
export async function openSessionFile(
  ...
  const root = await realpath(source.path).catch(() => {
    throw new ApiError(409, "session_file_missing");
  });
  const path = await realpath(session.path).catch(() => {
    throw new ApiError(409, "session_file_missing");
  });
  if (
    root !== source.path ||
    path !== session.path ||
    !withinSource(root, path)
  )
  ...
  try {
    ...
    if (
      !initial.isFile() ||
      `${initial.dev}:${initial.ino}` !== session.fileIdentity ||
      initial.size < session.cursor
    )
      throw new ApiError(409, "session_file_changed");
    if (
      initial.mtimeMs !== session.fileMtime ||
      initial.size !== session.size
    ) {
      const digest = createHash("sha256"),
        buffer = Buffer.allocUnsafe(262144);
      let position = 0;
      while (position < session.cursor) {
        ...
        const { bytesRead } = await handle.read(
          buffer,
          0,
          Math.min(buffer.length, session.cursor - position),
          position,
        );
      ...
      if (digest.digest("hex") !== session.prefixHash)
        throw new ApiError(409, "session_file_changed");
    }
    return {
      handle,
      async validate() {
        ...
        if (
          ...
          (await realpath(session.path)) !== path ||
          (await realpath(source.path)) !== root
        )
          throw new ApiError(409, "session_file_changed");
      },
    };
...
export async function readSessionEvents(
  ...
  if (!source?.enabled || !source.captureBodies || !events.length)
    return result;
  ...
  try {
    for (const event of result) {
      ...
      if (
        length <= 0 ||
        length > 4 * 1024 * 1024 ||
        event.endOffset > session.cursor
      )
        throw new ApiError(409, "session_file_changed");
      ...
      if (
        hash(`${session.id}:${session.generation}:${parsed.sourceKey}`).slice(
          0,
          40,
        ) !== event.id
      )
...
export async function matchingOffsets(
  session: Session,
  source: CollectionSource,
  query: string,
  context?: WorkContext,
    ...
    while (position < session.cursor && !found) {
      ...
      while ((boundary = pending.indexOf(10)) >= 0) {
        const line = pending.subarray(0, boundary),
          end = position - pending.length + boundary + 1;
        if (!skipping && line.length < 4 * 1024 * 1024) {
          try {
            if (
              parseSessionLine(
                line.toString("utf8").replace(/\r$/, ""),
                session.agent,
                lineStart,
              )
      ...
      if (pending.length > 4 * 1024 * 1024 || skipping) {
        skipping = true;
        pending = Buffer.alloc(0);
      }
