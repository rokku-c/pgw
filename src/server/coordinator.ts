...
export function decideContinuation(input: ContinuationInput): ContinuationDecision {
  if (input.mode === "off") return { decision: "stop", reason: "coordinator_disabled" };
  if (input.userStopped) return { decision: "stop", reason: "user_stopped" };
  if (input.approvalsPending) return { decision: "stop", reason: "approval_pending" };
  if (input.turnStatus !== "completed") return { decision: "stop", reason: `turn_${input.turnStatus}` };
  if (input.completionComplete) return { decision: "stop", reason: "criteria_satisfied" };
  if (!input.hasToolActivity) return { decision: "stop", reason: "no_tool_activity" };
  if (input.mode === "suggest") return { decision: "suggest", reason: "tool_result_needs_review" };
  return {
    decision: "continue",
    reason: "tool_result_needs_model_continuation",
    message: "[Gateway Coordinator] 最近的工具调用已返回，但目标尚未确认完成。请先检查工具结果和当前状态；如果仍有安全且必要的下一步，请继续执行，否则明确说明阻塞或完成依据。不要重复已经完成的副作用操作。",
  };
}
