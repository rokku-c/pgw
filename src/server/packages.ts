...
export function safePackagePath(path: string) {
  if (
    ...
    throw new ApiError(400, "unsafe_package_path");
  return path;
}
export function packageHash(
  files: Pick<PackageFile, "path" | "hash" | "size" | "executable" | "mode">[],
  directories?: { path: string; mode: number }[],
) {
  const entries = [...files]
    .sort((a, b) => a.path.localeCompare(b.path))
    .map(({ path, hash, size, executable, mode }) => ({
      path,
      hash,
      size,
      executable,
...
export async function readPackage(
  ...
  while (queue.length) {
    ...
    directories.push({
      path: relative(canonical, directory).split(sep).join("/") || ".",
      mode: (await lstat(directory)).mode & 0o777,
    });
    if (directories.length > 1000)
      throw new ApiError(413, "package_directory_limit");
    const entries = await readdir(directory, { withFileTypes: true });
    for (const entry of entries) {
      const path = join(directory, entry.name),
        rel = safePackagePath(relative(canonical, path).split(sep).join("/"));
      if (entry.isSymbolicLink())
        throw new ApiError(409, "package_symlink_requires_review");
      ...
      if (!within(canonical, resolved))
        throw new ApiError(403, "package_path_escape");
      ...
      try {
        ...
        if (before.nlink > 1)
          throw new ApiError(409, "package_hardlink_requires_review");
        if (
          !before.isFile() ||
          before.size > 4 * 1024 * 1024 ||
          bytes + before.size > 8 * 1024 * 1024 ||
          files.length >= 500
        )
          throw new ApiError(413, "package_size_limit");
        ...
        if (
          ...
          throw new ApiError(409, "package_changed_during_read");
  ...
  return { files, bytes, directories, hash: packageHash(files, directories) };
}
export async function writePackage(
  ...
  if (packageHash(pkg.files, pkg.directories) !== pkg.hash)
    throw new ApiError(409, "snapshot_integrity_error");
  await mkdir(root, { recursive: false, mode: 0o700 });
  for (const directory of pkg.directories || []) {
    if (directory.path !== ".")
      await mkdir(join(root, safePackagePath(directory.path)), {
        recursive: true,
        mode: 0o700,
      });
  }
  ...
  for (const file of pkg.files) {
    await context?.check();
    const path = join(root, safePackagePath(file.path));
  ...
  for (const directory of [...(pkg.directories || [])].sort(
    (a, b) => b.path.length - a.path.length,
  ))
    await chmod(
      directory.path === "."
        ? root
        : join(root, safePackagePath(directory.path)),
...
export function skillMetadata(content: string) {
  ...
  if (front) {
    try {
      const parsed = Bun.YAML.parse(front[1]);
      if (parsed && typeof parsed === "object" && !Array.isArray(parsed))
        metadata = parsed as Record<string, unknown>;
      else warnings.push("invalid_frontmatter");
    } catch {
      warnings.push("invalid_frontmatter");
    }
  ...
  const tools =
    typeof allowed === "string"
      ...
      : Array.isArray(allowed)
        ? allowed.filter((v) => typeof v === "string")
        : [];
  return {
