...
import { configDatabasePath, home } from "./config";
...
export async function storageStatus() {
  ...
  try {
    ...
    const free = (
      database.query("PRAGMA freelist_count").get() as {
        freelist_count: number;
      }
    ).freelist_count;
    const configPageSize = (
      database.query("PRAGMA config.page_size").get() as { page_size: number }
    ).page_size;
    const configPages = (
      database.query("PRAGMA config.page_count").get() as { page_count: number }
    ).page_count;
    const configFree = (
      database.query("PRAGMA config.freelist_count").get() as {
        freelist_count: number;
      }
    ).freelist_count;
    const configBytes = configPages * configPageSize;
    const configFreeBytes = configFree * configPageSize;
    const configWalBytes = await stat(`${configDatabasePath}-wal`)
      .then((info) => info.size)
      .catch(() => 0);
    return {
      path: filename,
      ...
      walBytes: await stat(`${filename}-wal`)
        .then((info) => info.size)
        .catch(() => 0),
      configPath: configDatabasePath,
      configBytes,
      configFreeBytes,
      configWalBytes,
      totalBytes: pages * pageSize + configBytes,
    };
  } finally {
    database.close();
  }
...
export async function compactStorage(
  progress: (value: Record<string, unknown>) => void = console.log,
) {
  ...
  try {
    ...
    const before = await storageStatus(),
      space = await statfs(home);
    if (space.bavail * space.bsize < before.bytes * 1.2)
      throw new Error(
        "Insufficient temporary space for safe SQLite compaction",
      );
    database.run("PRAGMA busy_timeout = 1000");
    const version = (
      database
        .query("SELECT max(version) version FROM schema_versions")
        .get() as { version: number }
    ).version;
    if (version < 12 || version > SCHEMA_VERSION)
      throw new Error("Start the updated gateway once before compacting");
    ...
    database.transaction(() => {
      for (const trigger of [
        ...
        database.run(`DROP TRIGGER IF EXISTS ${trigger}`);
      database.run("DROP TABLE IF EXISTS session_search");
      database
        .query(
          "INSERT OR IGNORE INTO schema_versions(version,appliedAt) VALUES(13,?)",
        )
        .run(Date.now());
    })();
    const sources = new Map(
      (
        database
          .query("SELECT * FROM collection_sources")
          .all() as CollectionSource[]
      ).map((source) => [source.id, source]),
    );
    const sessions = database
      .query("SELECT * FROM sessions WHERE captured=1")
      .all() as Session[];
    let cleared = 0,
      preserved = 0;
    for (const [index, session] of sessions.entries()) {
      const source = session.sourceId
        ...
        : undefined;
      if (!source?.enabled || !source.captureBodies || !session.prefixHash) {
        preserved++;
        continue;
      }
      ...
      try {
        try {
          file = await openSessionFile({ ...session, fileMtime: -1 }, source);
          await file.validate();
        } catch {
          preserved++;
          progress({ phase: "preserve_unavailable", session: session.id });
          continue;
        }
        cleared += database
          .query(
            "UPDATE session_events SET text=NULL WHERE sessionId=? AND endOffset<=? AND text IS NOT NULL",
          )
          .run(session.id, session.cursor).changes;
      } finally {
        await file?.handle.close();
      }
      progress({
        phase: "remove_duplicate_bodies",
        processed: index + 1,
    ...
    const after = {
      path: before.path,
      bytes: (await stat(before.path)).size,
      freeBytes: 0,
      walBytes: 0,
    };
    progress({
...
const STORAGE_CATEGORIES: { id: string; tables: string[] }[] = [
  {
    id: "sessions",
    tables: ["sessions", "session_events", "session_tombstones"],
  },
  { id: "captures", tables: ["request_capture_parts", "request_captures"] },
  {
    ...
    tables: [
      ...
      "trajectory_call_context",
    ],
  },
  {
    id: "jobs",
    tables: ["background_jobs", "debug_attempts", "jobs_dedup", "jobs_queue"],
  },
...
function ownerTable(name: string, known: string[]) {
  ...
  const stripped = name.startsWith("sqlite_autoindex_")
    ? name.slice("sqlite_autoindex_".length).replace(/_\d+$/, "")
    : name;
  if (known.includes(stripped)) return stripped;
  return (
    known
      .filter((table) => name.startsWith(`${table}_`))
...
export async function storageUsage(): Promise<StorageUsage> {
  const database = new Database(join(home, "gateway.sqlite"), {
    readonly: true,
    strict: true,
  });
  attachConfig(database);
  try {
    const rows = [
      ...(database
        .query("SELECT name,SUM(pgsize) bytes FROM main.dbstat GROUP BY name")
        .all() as { name: string; bytes: number }[]),
      ...(database
        .query("SELECT name,SUM(pgsize) bytes FROM config.dbstat GROUP BY name")
        .all() as { name: string; bytes: number }[]),
    ];
    const known = [...DATA_TABLES, ...CONFIG_TABLES];
    const categories = STORAGE_CATEGORIES.map((category) => ({
      id: category.id as StorageCategory,
      bytes: 0,
    }));
    ...
    for (const row of rows) {
      ...
      const category = table
        ? STORAGE_CATEGORIES.find((item) => item.tables.includes(table))
        : undefined;
      if (category)
        byId.get(category.id as StorageCategory)!.bytes += row.bytes;
...
export async function purgeStorage(ids: string[]) {
  const selected = STORAGE_CATEGORIES.filter((category) =>
    ids.includes(category.id),
  );
  if (!selected.length) throw new ApiError(400, "nothing_selected");
  const before = await storageStatus();
  const existing = new Set(
    (await db.query("SELECT name FROM sqlite_master WHERE type='table'")).map(
      (row: any) => row.name as string,
    ),
  );
  await db.transaction(async (manager) => {
    for (const table of selected.flatMap((category) => category.tables))
      if (existing.has(table)) await manager.query(`DELETE FROM ${table}`);
  });
  await audit("storage.purged", "local", {
    categories: selected.map((category) => category.id),
  });
  const after = await storageStatus();
  return {
    freedBytes: Math.max(0, after.freeBytes - before.freeBytes),
    bytes: after.bytes,
    categories: selected.map((category) => category.id),
  };
}
