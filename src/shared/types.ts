...
export type WireProtocol =
  "responses" | "chat" | "messages" | "gemini" | "systemone";
/** 互转时被丢弃的内容。`count` 在能界定边界的协议里是真实块数，chat/gemini 的匿名增量每类型至多记 1。 */
export interface ConversionDrop {
  type: string;
  count: number;
}
...
export type PublicProvider = Omit<Provider, "secretCipher"> & {
  hasSecret: boolean;
};
...
export interface ModelRoute extends RecordBase {
  ...
  targets: Target[];
  enabled: boolean;
  strategy: "priority" | "round_robin" | "least_active";
  inputPrice: number | null;
  outputPrice: number | null;
  cacheReadPrice: number | null;
  cacheWritePrice: number | null;
  cacheWriteLongPrice: number | null;
  contextLimit: number;
  outputLimit: number;
}
...
export interface Traffic extends RecordBase {
  ...
  patchIds: string[];
  runId: string | null;
  accounting: "pending" | "reported" | "estimated" | "unknown" | "rejected";
  pricing: {
    input: number | null;
    output: number | null;
...
export interface Asset extends RecordBase {
  ...
  status: "discovered" | "available" | "unavailable";
...
export interface Session extends RecordBase {
  ...
  size: number;
  cursor: number;
  status: "indexed" | "partial" | "error" | "missing" | "restricted";
...
export interface SessionEvent extends RecordBase {
  ...
  origin: "user" | "assistant" | "tool" | "system" | "unknown";
  text: string | null;
  metadata: Record<string, unknown>;
  offset: number;
  endOffset: number;
...
export interface PreferenceEvidence extends RecordBase {
  ...
  excerpt: string;
  kind: "explicit" | "inferred" | "correction" | "counterexample";
  confidence: number;
}
...
export interface SessionTimeline {
  ...
  total: number;
  next: number | null;
  branches: { id: string; count: number }[];
}
...
export type PublicJob = Omit<
  BackgroundJob,
  "payloadCipher" | "resultCipher" | "dedupKey"
>;
export interface AssetRoot extends RecordBase {
  name: string;
  path: string;
  agent: "claude" | "codex" | "pi" | "shared";
  project: string | null;
  enabled: boolean;
...
export interface AssetSnapshot extends RecordBase {
  assetId: string;
  hash: string;
  bytes: number;
  files: { path: string; hash: string; size: number; executable: boolean }[];
...
export interface AssetDeployment extends RecordBase {
  ...
  afterHash: string;
  beforeCipher: string | null;
  afterCipher: string;
  stagePath: string | null;
  error: string | null;
  diff: { path: string; action: "add" | "change" | "remove" }[];
}
...
export interface GatewayPolicies {
  adaptiveContext: AdaptiveContextPolicy;
  protocolConversion: boolean;
  transparentRetry: RetryPolicy;
}
...
export type CaptureStage =
  "request" | "effective" | "upstream" | "response" | "output";
...
export interface StorageUsage {
  ...
  configBytes: number;
  otherBytes: number;
  categories: { id: StorageCategory; bytes: number }[];
}
export interface CaptureStageUsage {
  stage: CaptureStage;
  bytes: number;
  chunks: number;
}
...
export interface ObservabilityEvent {
  ...
  at: number;
  kind:
    "session" | "model_call" | "managed_session" | "tool_call" | "job" | "scan";
  title: string;
  status: string;
  sessionId: string | null;
  runId: string | null;
