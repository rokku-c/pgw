...
const files=(await readdir("src/web")).filter(file=>/\.(ts|tsx)$/.test(file));
const source=(await Promise.all(files.map(file=>Bun.file(`src/web/${file}`).text()))).join("\n");
for(const key of keys){
  ...
  assert.ok(source.includes(`"${key}"`)||source.includes(`'${key}'`),`Unused locale key: ${key}`);
}
console.log(`${keys.length} keys · zh-CN / en-US · verified`);
